...
@pytest.mark.parametrize("quad", [
    [[-999, -999], [9999, -999], [9999, 9999], [-999, 9999]],  # 罩住整张图
    [[2000, 100], [3000, 100], [3000, 200], [2000, 200]],      # 整个落在右侧图外
    [[-3000, 100], [-2000, 100], [-2000, 200], [-3000, 200]],  # 整个落在左侧图外
    [[100, -900], [200, -900], [200, -800], [100, -800]],      # 整个落在上方图外
])
def test_bbox_stays_within_unit_square(quad):
    """四条边各自都要截到 0~1，且截完不能左右上下颠倒。"""
    x1, y1, x2, y2 = _quad_to_norm_bbox(quad, 640, 480)
