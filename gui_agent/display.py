...
系统缩放不影响截到多大，只影响界面元素的显示大小——前提是进程声明了 DPI 感知。
不声明的话系统 API 返回按缩放比例虚拟化过的尺寸，和实际截到的图对不上，175% 下
会把 1280×720 报成 2240×1260。captured_size() 先调 _ensure_dpi_aware() 再问 mss，
拿到的是真实物理像素。

切换后仍用 captured_size() 核对，不要假定设了就是那个值。
