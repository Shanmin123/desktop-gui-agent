...
误差为零说明整条链路没有丢精度。**只移动鼠标，不点击**：点击会触发屏幕上任何
一个当前显示的按钮，而移动是可逆的，足以验证坐标是否正确。

跑完会把鼠标放回原来的位置。随时可以把鼠标甩到屏幕左上角强制中断（FAILSAFE）。
...
def pick_spread(elements, n):
    ...
    按 3x3 网格分桶轮流取，比直接取前 n 个更能暴露边角上的坐标问题——
    如果存在缩放系数没对齐的问题，误差会随着离原点的距离线性放大。
    """
    buckets = {}
    for e in elements:
        cx, cy = e.center()
...
def move_and_verify(backend, want, retries: int = 3, settle: float = 0.05):
    ...
    需要重试是因为光标可能被别的程序抢走：实测在有游戏运行时，25 个点里有 15 个
    读回的位置对不上，而单独跑纯移动测试时三种移动方式都是零误差。所以一次读数
    对不上不代表坐标算错了，先重试再判定。
    """
    got = None
