...
DESC = {
 ...
 "tests/test_models.py": "大模型调用接口：输出解析、定位坐标空间、本地与 API 后端选择、故障注入",
 "tests/test_display.py": "屏幕分辨率：切换与还原、系统缩放检测、跨平台退化",
 "tests/test_chain.py": "LangChain 链：提示词模板、输出解析器、链式组装、两段式定位、提示词变体",
 "tests/test_planner.py": "任务拆解与规划：拆解解析、子任务反思、循环推进、提示词防抄袭",
...
buf = io.StringIO()
buf.write("# 单元测试报告\n\n对应大纲第 2、4 周交付物，第 5、6 周新增的模块同步补测。\n\n")
