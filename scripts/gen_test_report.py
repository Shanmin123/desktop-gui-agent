...
DESC = {
 ...
 "tests/test_planner.py": "任务拆解与子任务反思：拆解输出的解析、反思三态、重拆上限；以及两条长链路用例——16 步走完 4 个子任务含两次重拆，15 步不开拆解验证历史累积与卡住检测不误触发",
 ...
 "tests/test_train_lora.py": "LoRA 训练：提示词屏蔽、标签对齐、只算回答段的 logits、LoRA 目标自动发现、对齐层全量训练的模块定位、按验证损失取最优步、显存与步数计算",
 ...
 "tests/test_action_metrics.py": "动作指标：操作 F1 的 macro 与 micro、键盘动作要比输入内容、步级成功率的距离阈值",
